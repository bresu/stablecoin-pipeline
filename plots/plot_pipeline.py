...
def find_file(
    ...
    """Find an aggregation CSV.

    With file_prefix configured, use the exact flat-output filename generated by
    pipeline.py (for example usdt_eoa_eoa_monthly_activity.csv). Without a
    prefix, retain compatibility with the older per-dataset-folder layout.
    """
    if prefix:
...
def human_symmetric_axis(values: Iterable[float], title: str) -> dict:
    """Human-readable K/M/B ticks for charts containing positive and negative values."""
    numeric = pd.to_numeric(pd.Series(list(values)), errors="coerce").dropna()
    if numeric.empty:
        return {"title": title, "rangemode": "tozero"}

    max_abs = float(numeric.abs().max())
    positive_ticks = nice_tick_values(max_abs)
    negative_ticks = [-value for value in reversed(positive_ticks[1:])]
    ticks = negative_ticks + positive_ticks
    return {
        "title": title,
        "tickmode": "array",
        "tickvals": ticks,
        "ticktext": [human_number(value) for value in ticks],
        "range": [-max_abs * 1.08, max_abs * 1.08] if max_abs > 0 else None,
        "zeroline": True,
        "zerolinewidth": 1,
    }
...
def plot_monthly_activity(input_folder: Path, output_folder: Path, cfg: dict) -> None:
    ...
    counts_identical = (
        has_transactions
        and df["transaction_count"].fillna(-1).equals(df["transfer_count"].fillna(-1))
    )

    # For ERC-20 datasets, transfer events and Ethereum transactions are different
    # concepts and are both shown. For native ETH top-level transfers they are
    # identical, so only the transaction line is drawn to avoid exact overlap.
    ...
    if has_transactions:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="lines",
                yaxis="y2",
                line=dict(color=transaction_color, width=2, dash="dash" if not counts_identical else "solid"),
                customdata=df["transaction_count"].map(human_number),
                hovertemplate="%{x|%b %Y}<br>Transactions: %{customdata}<extra></extra>",
            )
        )
    else:
        print(f"[warn] {cfg.get('file_prefix', cfg['name'])}: transaction_count column not found")
...
def plot_monthly_mint_burn(input_folder: Path, output_folder: Path, cfg: dict) -> None:
    """Plot zero-address minting/burning separately from economic transfers."""
    df = read_csv(input_folder, "monthly_mint_burn", cfg)
    if df is None or df.empty:
        return

    required = {"minted_volume", "burned_volume", "net_issuance", "cumulative_net_issuance"}
    missing = required.difference(df.columns)
    if missing:
    ...
    df = df.copy()
    df["burned_volume_negative"] = -pd.to_numeric(df["burned_volume"], errors="coerce").fillna(0)
    ...
    fig.add_trace(
        go.Bar(
            ...
            name="Minted volume",
            marker_color=primary,
            customdata=df["minted_volume"].map(human_number),
    ...
    fig.add_trace(
        go.Bar(
            x=df["month"],
            y=df["burned_volume_negative"],
            name="Burned volume",
            marker_color=secondary,
            customdata=df["burned_volume"].map(human_number),
    ...
    fig.add_trace(
        go.Scatter(
            ...
            name="Net issuance",
            mode="lines",
            line=dict(color=transaction_color, width=2),
            customdata=df["net_issuance"].map(human_number),
            hovertemplate="%{x|%b %Y}<br>Net issuance: %{customdata}<extra></extra>",
        )
    )

    axis_values = pd.concat(
        [df["minted_volume"], df["burned_volume_negative"], df["net_issuance"]],
        ignore_index=True,
    )
    fig.update_layout(
        title=f"{cfg['title_prefix']}: Monthly minting and burning",
        xaxis=dict(title="Month"),
        yaxis=human_symmetric_axis(axis_values, f"{cfg['token_symbol']} volume"),
        barmode="relative",
        hovermode="x unified",
        bargap=0.08,
    )
    save_fig(fig, output_folder / "monthly_mint_burn.html", cfg)

    cumulative = pd.to_numeric(df["cumulative_net_issuance"], errors="coerce")
    fig_cumulative = go.Figure()
    fig_cumulative.add_trace(
        go.Scatter(
            x=df["month"],
            y=cumulative,
            name="Cumulative net issuance",
            mode="lines",
            line=dict(color=primary, width=2.5),
            customdata=cumulative.map(human_number),
            hovertemplate=(
                "%{x|%b %Y}<br>Cumulative net issuance: %{customdata}<extra></extra>"
            ),
        )
    )
    axis = (
        human_symmetric_axis(cumulative, f"{cfg['token_symbol']} volume")
        if (cumulative < 0).any()
        else human_axis(cumulative, f"{cfg['token_symbol']} volume")
    )
    fig_cumulative.update_layout(
        title=f"{cfg['title_prefix']}: Cumulative net issuance from mint/burn events",
        xaxis=dict(title="Month"),
        yaxis=axis,
        hovermode="x unified",
    )
    save_fig(fig_cumulative, output_folder / "cumulative_net_issuance.html", cfg)
...
def plot_adoption(input_folder: Path, output_folder: Path, cfg: dict) -> None:
    ...
    fig.update_layout(
        title=f"{cfg['title_prefix']}: Address adoption",
        xaxis=dict(title="Month"),
...
FILTER_LABELS = {
    "all": "all transfers",
    "eoa_eoa": "EOA → EOA",
    "eoa_sc": "EOA → smart contract",
    "sc_eoa": "smart contract → EOA",
    "sc_sc": "smart contract → smart contract",
}
...
def expand_dataset_matrix(cfg: dict) -> list[dict]:
    """Expand compact token/filter configuration into plotting datasets."""
    ...
    filters = matrix.get(
        "filters",
        ["all", "eoa_eoa", "eoa_sc", "sc_eoa", "sc_sc"],
    )
    ...
    for token in matrix.get("tokens", []):
        ...
        prefix_symbol = _safe_prefix(symbol)
        for transfer_filter in filters:
            prefix = f"{prefix_symbol}_{transfer_filter}"
            ...
            datasets.append(
                {
                    ...
                    "show_mint_burn": transfer_filter == "all",
                }
            )
    ...
    if native.get("enabled", False):
        ...
        datasets.append(
            {
                "name": f"{symbol} native top-level transfers",
                "token_symbol": symbol,
                ...
                "bucket_color_sequence": native.get("bucket_color_sequence"),
...
def run_plot_pipeline() -> None:
    ...
    # Explicit datasets remain supported. The compact dataset_matrix is useful
    # for the final run with 13 tokens × 5 transfer categories plus native ETH.
    datasets = list(cfg.get("datasets", []) or [])
    datasets.extend(expand_dataset_matrix(cfg))

    if not datasets:
        raise ValueError(
            "No datasets defined in config/plot_config.yaml "
            "(use datasets: or dataset_matrix:)"
        )

    print(f"[config] plotting {len(datasets)} datasets")
